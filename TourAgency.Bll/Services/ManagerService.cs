...
namespace TourAgency.Bll.Services
{
    public class ManagerService : TourService, IManagerService
    {
        ...
        public void Register(ManagerDTO model)
        {
            ApplicationUser user = _dataBase.UserManager.FindByEmail(model.User.Email);
            if (user == null)
            {
                var manager = MappingDTO.MapManager(model);
                _dataBase.Managers.Register(manager);
                _dataBase.Save();
                _dataBase.UserManager.AddToRole(model.User.Id, "manager");
                _dataBase.Save();
            }
        }
